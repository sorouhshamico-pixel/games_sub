...
export default async function GamesPage({
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10">
      <div>
        <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">{t("nav.games")}</h1>
        {search ? (
          <p className="mt-1 text-sm text-[var(--color-text-muted)]">
            {locale === "ar" ? `نتائج البحث عن "${search}"` : `Search results for "${search}"`}{" "}
            <Link href="/games" className="text-brand-primary underline">
              {locale === "ar" ? "مسح" : "clear"}
            </Link>
          </p>
        ) : null}
      ...
      <div className="flex flex-wrap gap-2" role="navigation" aria-label={t("nav.games")}>
        <Link
          ...
          className={`rounded-full border px-4 py-1.5 text-sm ${
            !category ? "border-brand-primary bg-brand-primary/10 text-brand-primary" : "border-[var(--color-border)] text-[var(--color-text-primary)]"
          }`}
        >
          {locale === "ar" ? "الكل" : "All"}
        </Link>
        {categories.map((c) => (
          <Link
            ...
            className={`rounded-full border px-4 py-1.5 text-sm ${
              category === c.slug ? "border-brand-primary bg-brand-primary/10 text-brand-primary" : "border-[var(--color-border)] text-[var(--color-text-primary)]"
            }`}
          >
            {c.name}
          </Link>
          ...
          <div className="flex flex-wrap gap-3">
            {brands.map((brand) => (
              <Link
                ...
                className="flex items-center gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-2 text-sm font-medium text-[var(--color-text-primary)] hover:border-brand-primary/60"
              >
